...
# change origin value to factor data type
...
growth_data<-growth_data%>%select(Origin,Variety,`[DIN] 10/17`,RGR)%>%group_by(Variety,Origin)%>% summarise(mean_nitrogen = mean(`[DIN] 10/17`),mean_RGR=mean(RGR))